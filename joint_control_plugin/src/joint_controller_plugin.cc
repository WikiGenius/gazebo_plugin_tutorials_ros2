...
public:
  void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
    ...
    std::cout << "Joint By Number =" << this->jointByNum->GetScopedName()
              << std::endl;
    std::cout << "Joint By Name   =" << this->jointByName->GetScopedName()
              << std::endl;
    ...
    // bind- we don't have define callback fn input parametes its replace by
    // placeholder
  }
...
public:
  void OnUpdate() {
    ...
    // method 3
    if (count == 0) {
      // reset joint controller
      this->jointController.reset(new physics::JointController(this->model));
      // add a joint to control
      this->jointController->AddJoint(this->model->GetJoint("joint_1"));
      // get full joint name
      std::string name = this->model->GetJoint("joint_1")->GetScopedName();
      // set pid for the joint
      this->jointController->SetVelocityPID(name, common::PID(1, 0, 0));
      // set target velocity for the joint
      this->jointController->SetVelocityTarget(name, 0.1);
...
public:
  physics::JointControllerPtr
      jointController; // pointer to a joint controller for PID
