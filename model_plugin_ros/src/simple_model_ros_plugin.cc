...
public:
  void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
    ...
    this->sub = this->nh.subscribe(
        "/model_move_up", 10, &SimpleModelRosPlugin::Activate_Callback, this);
    //                             topic name , queue size, callback, this
    //                             pointer(to access class member fn)
    ...
    {
      this->vel = _sdf->Get<double>(
          "model_vel"); // use _sdf pointer & Get to find value in <model_vel>
    }
    ...
    //                                          OnUpdate i.e this->OnUpdate
  } //                         bind- we don't have define callback fn input
    //                         parametes its replace by placeholder
