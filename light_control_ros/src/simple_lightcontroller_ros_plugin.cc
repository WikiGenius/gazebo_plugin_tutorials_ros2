#include <gazebo/common/common.hh> // For common Gazebo functions like ModelPlugin, event, and GetName
...
namespace gazebo {
class SimpleLightControllerROSPlugin : public ModelPlugin {
public:
  SimpleLightControllerROSPlugin() : ModelPlugin() {}

  void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) override {
    // Ensure ROS 2 is initialized
    if (!rclcpp::ok()) {
      RCLCPP_FATAL(
          rclcpp::get_logger("SimpleLightControllerROSPlugin"),
          "A ROS 2 node for Gazebo has not been initialized. Exiting.");
      return;
    }

    RCLCPP_INFO(rclcpp::get_logger("SimpleLightControllerROSPlugin"),
    ...
    rclcpp::QoS qos_profile(rclcpp::KeepLast(10)); // Keep the last 10 messages
    qos_profile.transient_local(); // Durability: TRANSIENT_LOCAL

    // Create a subscriber to the /light_color topic with the custom QoS profile
    this->sub_light_ =
        this->ros_node_->create_subscription<std_msgs::msg::String>(
            "/light_color", qos_profile,
            std::bind(&SimpleLightControllerROSPlugin::ColourCallback, this,
                      std::placeholders::_1));
    ...
    this->update_connection_ = event::Events::ConnectWorldUpdateBegin(
        std::bind(&SimpleLightControllerROSPlugin::OnUpdate, this));
    ...
    std::cout << "Complete light name: " << this->complete_light_name_
              << std::endl;
  }
...
private:
  ...
  std::string model_name_, link_name_,
      light_name_;                  // Names of the model, link, and light
  std::string complete_light_name_; // Full name of the light in Gazebo

  transport::PublisherPtr
      light_pub_; // Gazebo transport publisher for light modifications
  event::ConnectionPtr
      update_connection_; // Pointer to the update event connection

  // ROS 2 members
  rclcpp::Node::SharedPtr ros_node_; // ROS 2 node
  rclcpp::Subscription<std_msgs::msg::String>::SharedPtr
      sub_light_;                 // ROS 2 subscriber
  std::string light_colour_name_; // Current light color from ROS topic
};

// Register this plugin with Gazebo
GZ_REGISTER_MODEL_PLUGIN(SimpleLightControllerROSPlugin)
} // namespace gazebo
